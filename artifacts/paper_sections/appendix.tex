...
\centering
\small
\caption{C1 negative control magnitude comparison. Python-X = mean CKA across (python--svg, python--tikz, python--asy); Visual-X = mean CKA across (svg--tikz, svg--asy, tikz--asy). Ratio = Python-X / Visual-X. M1 criterion requires Ratio $< 0.50$; \textbf{bold} = the sole passing cell.}
\label{tab:c1_python_magnitude}
\begin{tabular}{llcccc}
\toprule
Model & Layer & Py-X CKA & Vis-X CKA & Ratio & M1 \\
\midrule
Coder-7B & L4  & 0.093 & 0.087 & 1.06 & fail \\
Coder-7B & L8  & 0.088 & 0.091 & 0.98 & fail \\
Coder-7B & L12 & 0.092 & 0.121 & 0.76 & fail \\
Coder-7B & L16 & 0.089 & 0.141 & 0.63 & fail \\
Coder-7B & L20 & 0.084 & 0.128 & 0.65 & fail \\
Coder-7B & L24 & 0.092 & 0.141 & 0.65 & fail \\
Coder-7B & L28 & 0.102 & 0.149 & 0.69 & fail \\
\midrule
VisCoder2-7B & L4  & 0.093 & 0.088 & 1.05 & fail \\
VisCoder2-7B & L8  & 0.091 & 0.095 & 0.97 & fail \\
VisCoder2-7B & L12 & 0.099 & 0.132 & 0.75 & fail \\
VisCoder2-7B & L16 & 0.096 & 0.143 & 0.67 & fail \\
VisCoder2-7B & L20 & 0.086 & 0.130 & 0.66 & fail \\
VisCoder2-7B & L24 & 0.087 & 0.145 & 0.60 & fail \\
VisCoder2-7B & L28 & 0.091 & 0.137 & 0.66 & fail \\
\midrule
Qwen2.5-base & L4  & 0.094 & 0.085 & 1.11 & fail \\
Qwen2.5-base & L8  & 0.087 & 0.089 & 0.98 & fail \\
Qwen2.5-base & L12 & 0.088 & 0.112 & 0.78 & fail \\
Qwen2.5-base & L16 & 0.079 & 0.115 & 0.68 & fail \\
Qwen2.5-base & L20 & 0.071 & 0.109 & 0.65 & fail \\
Qwen2.5-base & L24 & 0.075 & 0.121 & 0.62 & fail \\
Qwen2.5-base & L28 & \textbf{0.030} & \textbf{0.076} & \textbf{0.39} & \textbf{pass} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Note:} This ablation is a post-hoc exploratory analysis (D075), not a pre-registered prediction.
...
We extract hidden states at three depth-equivalent layers (L4/L18/L31 out of 32, corresponding to 13\%/56\%/97\% depth) for all 252 triples and 252 Python snippets, computing CKA with 5{,}000 permutations and 1{,}000 bootstrap subsamples.

\begin{table}[!ht]
\centering
\small
\caption{Llama-3-8B baseline CKA: visual cross-format pairs (top) and python--visual pairs (bottom). Bold $p$-values indicate $p < 0.005$.}
\label{tab:llama_baseline}
\begin{tabular}{lccc}
\toprule
Pair & L4 (13\%) & L18 (56\%) & L31 (97\%) \\
\midrule
\multicolumn{4}{l}{\emph{Visual cross-format pairs}} \\
SVG--TikZ   & 0.053 (\textbf{p=.002}) & 0.092 (\textbf{p<.001}) & 0.105 (\textbf{p<.001}) \\
SVG--Asy    & 0.075 (\textbf{p<.001}) & 0.101 (\textbf{p<.001}) & 0.105 (\textbf{p<.001}) \\
TikZ--Asy   & 0.126 (\textbf{p<.001}) & 0.211 (\textbf{p<.001}) & 0.202 (\textbf{p<.001}) \\
\midrule
\multicolumn{4}{l}{\emph{Python--visual pairs}} \\
Python--SVG  & 0.060 (p=.184) & 0.062 (p=.587) & 0.064 (p=.520) \\
Python--TikZ & 0.085 (p=.182) & 0.101 (p=.161) & 0.118 (\textbf{p=.004}) \\
Python--Asy  & 0.107 (p=.118) & 0.115 (p=.100) & 0.128 (\textbf{p=.002}) \\
\bottomrule
\end{tabular}
\end{table}

Visual cross-format CKA is significant at all 9 cells ($p \leq 0.002$), with magnitudes comparable to the Qwen family (mean 0.119 vs.\ Qwen 0.119--0.154).
The TikZ--Asy $>$ SVG--Asy $>$ SVG--TikZ ordering replicates perfectly, strengthening the token-overlap gradient (F3) as robust to the absence of code-specialized pretraining, at least for Llama-3-8B.
In contrast, python--visual pairs are overwhelmingly non-significant (7/9 cells $p > 0.10$); only Python--TikZ and Python--Asy at L31 reach marginal significance ($p = 0.004$, $p = 0.002$), which may reflect generic final-layer feature convergence rather than meaningful cross-format structure.

This split---visual CKA persists without code pretraining while python--visual entanglement does not---suggests that the C1 failure mode (python yielding 60--110\% of visual CKA in code-pretrained models) is specific to code-pretrained LLMs, not a general property of language models.
