...
However, a code-naive ablation (Llama-3-8B) shows that visual cross-format CKA persists without code pretraining (21/21 cells $p < 0.005$), while python--visual permutation significance splits between code-pretrained and code-naive models: Qwen family reaches significance in 9/21 cells (min-$p<0.05$, uncorrected), whereas Llama-3-8B reaches only 1/21; python/visual magnitude ratios are comparable across both families (0.60--1.11), so the split is in statistical detectability rather than raw magnitude.
