...
However, a code-naive ablation (Llama-3-8B) shows that visual cross-format CKA persists without code pretraining (21/21 cells $p < 0.005$), while python--visual \emph{permutation detectability} does not (20/21 cells $p > 0.05$); python/visual magnitude ratios are comparable across both families, so the split is in statistical detectability rather than raw magnitude.
