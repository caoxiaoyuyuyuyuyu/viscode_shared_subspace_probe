...
An exploratory ablation with code-naive Llama-3-8B across 7 equidistant layers (Appendix~\ref{sec:llama-baseline}) reveals that while visual cross-format CKA persists at comparable magnitude (21/21 cells $p < 0.005$), the python--visual entanglement does not arise (20/21 cells $p > 0.05$; the sole exception is python--Asy at L8 with borderline $p = 0.039$).
...
Thus, python--visual entanglement in code-naive Llama-3, if present, is \emph{layer-local} (confined to the deepest layer at 97\% depth), whereas in code-pretrained Qwen models, python-X significance is broader (9/21 cells $p<0.05$) with observed-to-null ratios consistently above baseline.
This constitutes a quantitative split in detectability rather than a qualitative split in magnitude.
The initial three-layer observation was post-hoc (D075); the 7-layer confirmation met pre-registered criteria (D078: 21/21 visual $\geq$ 70\%, 1/21 python $\leq$ 40\%).
