...
\paragraph{Cross-model comparison: shared magnitude, distinct permutation signature.}
A pre-registered ablation with code-naive Llama-3-8B (D078) across 7 equidistant layers (Appendix~\ref{sec:llama-baseline}) reveals that while visual cross-format CKA persists at comparable magnitude (21/21 cells $p < 0.005$), python--visual pairs fail to reach permutation significance (20/21 cells $p > 0.05$; sole exception: python--Asy L8, $p = 0.039$).
Following the initial exploratory observation (D075, three layers), we pre-registered confirmation criteria (D078: visual $\geq$70\% \& python $\leq$40\% significance) before the 7-layer scan; the result (visual 100\%, python 4.8\%) met these criteria.
The C1 magnitude pattern (python-X CKA at 60--110\% of visual-X) is shared by Llama-3-8B (ratio 0.60--0.99 across 7 layers); however, Llama's python-X observations are statistically indistinguishable from the permutation baseline (obs/null $\approx$ 1.0--1.1), whereas Qwen family python-X cells reach significance in 9/21 cells.
The split is thus in \emph{permutation detectability}, not magnitude.
...
\paragraph{Split hypothesis (D075 exploratory $\to$ D078 pre-registered confirmation).}
...
(2)~python--visual entanglement differs in statistical detectability: Qwen family python-X cells reach significance in 9/21 cells (min-$p<0.05$), whereas Llama-3-8B achieves only 1/21.
Both families show comparable python/visual magnitude ratios (0.60--1.11), but Llama's python-X values overlap with the permutation null, indicating that the entanglement signature requires code pretraining to emerge above baseline;
We note that the 7-layer equidistant protocol samples to L28 (87.5\% depth), missing the final layer (L31, 97\% depth) where the original three-layer scan (D075) found its only two significant python cells.
Thus, python--visual entanglement in code-naive Llama-3, if present, is \emph{layer-local} (present only at the deepest tested layer in D075, L31, 97\% depth), whereas in code-pretrained Qwen models, python-X significance is broader (9/21 cells $p<0.05$) with observed-to-null ratios consistently above baseline.
This constitutes a quantitative split in detectability rather than a qualitative split in magnitude.
The initial three-layer observation was exploratory (D075); the 7-layer confirmation met pre-registered criteria (D078: 21/21 visual $\geq$ 70\%, 1/21 python $\leq$ 40\%).
