...
This criterion fails at 20 of 21 cells: python-X CKA ranges from 60--110\% of visual-X CKA across models and layers (Appendix Table~\ref{tab:cka_complete}).
...
An exploratory ablation with code-naive Llama-3-8B (Appendix~\ref{sec:llama-baseline}; D075, post-hoc) reveals that while visual cross-format CKA persists at comparable magnitude (9/9 cells $p < 0.005$), the python--visual entanglement does not arise (7/9 cells $p > 0.10$).
...
An ablation with code-naive Llama-3-8B (Appendix~\ref{sec:llama-baseline}) suggests a two-track interpretation:
(1)~visual cross-format CKA persists at comparable magnitude without code pretraining (9/9 cells $p < 0.005$), indicating that visual code formats share representational structure in general-purpose LLMs;
(2)~python--visual entanglement is substantially stronger in code-pretrained models: Qwen family python-X CKA reaches 60--110\% of visual-X magnitude (D060), whereas Llama-3-8B python-X cells are overwhelmingly non-significant (7/9 cells $p > 0.10$), suggesting that the C1 failure mode is a code-pretraining artifact.
The marginal significance of Python--TikZ and Python--Asy at Llama's final layer ($p = 0.004$, $p = 0.002$) may reflect generic final-layer feature convergence rather than meaningful cross-format structure.
