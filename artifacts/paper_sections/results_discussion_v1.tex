...
We note that the 7-layer equidistant protocol samples to L28 (87.5\% depth), missing the final layers where the original three-layer scan (D075) found its only two significant python cells (L31, 97\% depth).
Thus, python--visual entanglement in code-naive Llama-3, if present, is \emph{layer-local} (confined to the deepest 1--2 layers at 97\% depth), whereas in code-pretrained Qwen models it is \emph{layer-general} (significant across the full depth range).
This constitutes a quantitative split in extent rather than a qualitative split in presence.
